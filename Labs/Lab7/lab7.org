...
*** Scalar product
#+begin_src python :results output :session
def scalar_product(m,A):
    """ Takes the scalar product of the scalar m and the matrix A. """
    n = len(A)
    m = len(A[0])
    return [[m * A[i][j] for j in range(m)] for i in range(n)]
#+end_src
...
A = [[1,2], [3,4]]
v = [[2], [3]]
print ("2 times A:", scalar_product(2, A))
print ("3 times v:", scalar_product(3, v))
#+end_src
