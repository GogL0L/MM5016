...
#+begin_src python :results output :session
def jacobi_next_value_function(D, D_inverse, L, U, b):
    """ This returns the function that generates the next vector
    approximation based on the formula: X_k-1 = D^(-1) (L+U)X^(k-1) + D^(-1)b,
    from the jacobi method.
    """
    L_plus_U = matrix_add(L, U)
    first_coefficient = matrix_multiply(D_inverse, L_plus_U)
    constant = D_inverse
#+end_src
